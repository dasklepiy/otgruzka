...
Процедура ЗаполнитьТаблицуКлиентов();
	...
	|	И ТоварыНаСкладахОстатки.НомерМестаОтгрузки.Отстойник = ЛОЖЬ
	|
	|УПОРЯДОЧИТЬ ПО
	|	Контрагент";
...
Процедура СоздатьПутевыеЛисты(Команда)
	Если Вопрос("Создать путевые листы за "+ Дата, РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Нет Тогда
		Возврат;	
	КонецЕсли;
