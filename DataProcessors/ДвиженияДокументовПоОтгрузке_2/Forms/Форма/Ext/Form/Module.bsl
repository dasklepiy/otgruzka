...
&НаКлиенте
 ...
 Процедура  СоздатьПогрузкаВАвтоСЕРВЕР(масСтрок)
	Для Каждого эл_масСтрок Из масСтрок Цикл
		...
		//-----------------------------------
		докПрихода=эл_масСтрок.ДокументПрихода;
		
		новыйДок=Документы.ПогрузкаТоваровВАвтомобиль.СоздатьДокумент();
		новыйДок.Дата=ТекущаяДата();
		новыйДок.Водитель=докПрихода.ПутевойЛист.Водитель;
		новыйДок.КоличествоКоробок=докПрихода.Коробки;
		новыйДок.ОбщийВес=докПрихода.ОбщийВес;
		новыйДок.ОбщийОбъем=докПрихода.ОбщийОбъем;
		//новыйДок.Подразделение=докПрихода.Подразделение;
		новыйДок.РаботникОхраны=ЭтаФорма.РаботникОхраны;
		
		новыйДок.Контрагент=докПрихода.Получатель;
		
		новыйДок.ПутевойЛист=докПрихода.ПутевойЛист;
		новыйДок.ДокументОснование=докПрихода;
		
		новыйДок.ДокументыРасхода.Загрузить(докПрихода.ДокументыРасхода.Выгрузить());
		новыйДок.МестаОтгрузки.Загрузить(докПрихода.МестаОтгрузки.Выгрузить());
