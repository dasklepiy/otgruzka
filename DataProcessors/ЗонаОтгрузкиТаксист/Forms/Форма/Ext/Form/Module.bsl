...
Функция ФФЗаписатьДокументSS(ТипРедактирования)
	...
	Если ОбъектДок.КонецОтгрузки>НачалоГода(ТекущаяДата()) и ОбъектДок.КонецОтгрузки<ТекущаяДата() тогда
		ОбъектДок.МестаОтгрузки.Очистить();
		ОбъектДок.КонецОтгрузки = Дата("00010101");
		ОбъектДок.ВозвратВодителя=Истина;
		ОбъектДок.Самовывоз=Истина;
		ОбъектДок.ДокументСписания="";
		ОбъектДок.НачалоОтгрузки=ТекущаяДата();
		ОбъектДок.ПутевойЛист="";
		ОбъектДок.ПогрузкаВАвто="";
		ОбъектДок.Списан=Ложь;
	КонецЕсли;	
			...
			Если Выборка.Количество()>0 Тогда 
				...
				Если ЗначениеЗаполнено(Объект.Док) тогда
					...
					Если С=0 Тогда
						...
						КолТары=КолТары+1;
						ОбДок.КолТара=КолТары;
						ОбДок.Записать();
						Объект.КолТара=ОбДок.КолТара;
...
&НаКлиенте
Процедура РеесторТара(Команда)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ДвижениеТарыВодительОстатки.УКого КАК УКого,
	               |	ДвижениеТарыВодительОстатки.Клиент,
	               |	ДвижениеТарыВодительОстатки.КолОстаток
	               |ИЗ
	               |	РегистрНакопления.ДвижениеТарыВодитель.Остатки КАК ДвижениеТарыВодительОстатки
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	ДвижениеТарыВодительОстатки.УКого.Наименование";
	
	//Запрос.УстановитьПараметр("", );
	
	Результат = Запрос.Выполнить();
	Выборка = Результат.Выбрать();
	Макет=Обработки.ЗонаОтгрузкиТаксист.ПолучитьМакет("Реестр");
	...
	ТД1.Вывести(Шапка);
	Пока Выборка.Следующий() Цикл
		Стр.Параметры.Водитель=Выборка.УКого ;
		Стр.Параметры.Аптека=Выборка.Клиент ;
		Стр.Параметры.Долг= Выборка.КолОстаток ;
		ТД1.Вывести(Стр);
	КонецЦикла;
	ТД1.Показать();
КонецПроцедуры
